...
% kernel = 'epanechnikov';
%% Loop
seedFolder = ['./mat/ts-averaged/',database];
freqsDir = dir('./mat/ts-averaged/suchai');  %pair only with suchi
...
for i = 1 : length(freqsDir)
    freq = freqsDir{i};
    saveFolder = ['./mat/pdf-averaged/',database,'/',freq];
    if ~isdir(saveFolder)
        mkdir(saveFolder)
    end
    
    currFreqFolder = strcat(seedFolder, '/', freq);
    ttauFolders = dir(currFreqFolder); %ttau folders
    ttauFolders = {ttauFolders.name};
    ttauFolders = ttauFolders(3:end);
    ttauFolders = sortn(ttauFolders);
    
    for j = 1 : length(ttauFolders)
       
        currTauString = ttauFolders{j};
        saveFolder = [saveFolder,'/',currTauString];
        tmpIdx = strfind(currTauString,'_');
        ttauValue = currTauString(tmpIdx+1:end);
        currTauFolder = strcat(currFreqFolder, '/', currTauString);
        matfiles = dir(currTauFolder); 
        matfiles = {matfiles.name};
        matfiles = matfiles(3:end);
        matfiles = sortn(matfiles);
        
        for kl = 1: length(matfiles)
            currentMatfile = matfiles{kl};
            currentMatfile = currentMatfile(1:(end-4));
            newMatFileName = strcat(saveFolder,'/', currentMatfile,'_pdfEstimator.mat');
            if exist(fullfile(newMatFileName),'file')
                disp([newMatFileName,' already exists']);
                continue
            end
            disp([newMatFileName,' is being created']);
            dateIndexes = strfind(matfiles, currentMatfile);
            emptyCells = cellfun ('isempty', dateIndexes);
            dateIndexes(emptyCells) = {0};
            dateIndexes = logical(cell2mat(dateIndexes));
            filesWithDate = strcat(currTauFolder,'/', matfiles(dateIndexes));
            for kj = 1 : length(filesWithDate)
                loadMyFile = filesWithDate{kj};
                S = load(loadMyFile);
                name = fieldnames(S);
